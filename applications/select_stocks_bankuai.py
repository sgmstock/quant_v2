...
# -*- coding: utf-8 -*-
"""
分类因子的选股系统（就是板块面的判断：这个板块不是行业板块，是风格类的板块）
支持分类因子的权重调整，适用于不同市场环境下的选股需求
第一是提前筛选：和市场板块轮动环境相关度极低的个股。避免技术好但板块不符合的情况。
第二是进行分类因子（板块因子）的评分

主要功能：
1. 支持所有分类因子（板块因子）：低价、小市值等，可根据市场来人工主观调整
2. 因子权重可人为调整
3. 支持房地产板块测试
4. 提前筛选：使用quantile(0.2)作为阈值，低于阈值的股票不进入选股的评分环节。
5. 作为板块面的分类因子，在后续选股中进行评分。
"""
